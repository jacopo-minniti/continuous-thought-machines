...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scan CIFAR validation split for CTM mistakes and cache metrics."
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="data/",
        help="Directory where torchvision will load/cache the dataset.",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="tasks/image_classification/analysis/outputs/cifar_mistakes",
...
def load_dataset(name: str, root: str):
    if name == "cifar10":
        ...
        mean = [0.5070751592371341, 0.48654887331495067, 0.4409178433670344]
        std = [0.2673342858792403, 0.2564384629170882, 0.27615047132568393]
        transform = transforms.Compose(
            [
                transforms.ToTensor(),
                transforms.Normalize(mean=mean, std=std),
...
def main():
    ...
    dataset, dataset_mean, dataset_std, class_labels = load_dataset(args.dataset, args.data_root)
    loader = DataLoader(
        dataset,
    ...
    if args.inference_iterations is not None:
        if model.iterations != args.inference_iterations:
            print(
                f"Overriding model iterations: {model.iterations} -> {args.inference_iterations}"
            )
    ...
    print(f"Evaluating {total_samples} samples from {args.dataset}.")
    ...
    with torch.inference_mode():
        ...
        for batch_idx, (inputs, targets) in enumerate(pbar):
            ...
            pbar.set_postfix(
                processed=processed,
                batches=math.ceil(processed / args.batch_size),
            )
